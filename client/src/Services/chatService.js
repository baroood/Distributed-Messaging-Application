...
export function usedeleteConversationMessages() {
    ...
    const deleteConversationMessages = id => {
        return fetch(
            `${
                process.env.REACT_APP_API_URL
            }/api/messages/conversations/delete?messageId=${id}`,
            requestOptions
        )
            .then()
            .catch(() =>
...
// edit messages using its id
export function useeditConversationMessages() {
    
    const editConversationMessages = (id, msg) => {
        const requestOptions = {
            method: 'POST',
            headers: authHeader(),
            body: JSON.stringify({ messageId: id, msg: msg}),
        };

        return fetch(
            `${
                process.env.REACT_APP_API_URL
            }/api/messages/conversations/edit`,
            requestOptions
        )
            .then()
            .catch(() =>
    ...
    return editConversationMessages;
}
